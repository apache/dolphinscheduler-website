...
* **MasterServer**
    ...
    - **MasterSchedulerThread** is a scan thread that periodically scans the **command** table in the database and performs different business operations according to different command types.
...
* **WorkerServer**
     ...
     The Worker node is responsible for balancing strategies: random, looping, and linear weighted load balancing of cpu and memory
       ...
       - **TaskPriorityQueuwConsumer** is primarily responsible for constantly picking up corresponding tasks from dispatc and called according to different tasks.
       Worker-TaskScheduleThread,send a confirmation message to ack after workerServer gets th task,response shows the result status of the worker get task.
       - **LoggerServer** is an RPC service that provides functions such as log fragment viewing, refresh and download.

     - **ZooKeeper**

       The ZooKeeper service，Both MasterServer and WorkerServer nodes in the system are managed by Zookeeper for cluster management and fault tolerance.In addittion ,the system is based on ZooKeeper for event monitoring,service,registration,heartbeat,distributed lock.
