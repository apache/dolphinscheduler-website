...
3. 编写单元测试代码应遵守 BCDE 原则，以保证被测试模块的交付质量。
...
4. 对于数据库相关的增删改查等操作，不能假设数据库里的数据是存在的，或者直接向数据库中插入数据，应当采用程序插入或者导入数据的方式准备数据。

   > 例如，在删除某一行数据的单元测试中，若事先手动向数据库中增加一行数据作为待删除目标，但由于该行新增数据并不一定符合业务插入规范，因此有可能导致测试结果异常。

5. 数据库相关的单元测试可以设定自动回滚机制，避免数据库中存在脏数据。或者对单元测试产生的数据有明确的前后缀标识。

6. 为了更方便地进行单元测试，业务代码应避免出现以下情况：
