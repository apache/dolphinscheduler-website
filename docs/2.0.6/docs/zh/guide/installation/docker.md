...
> PowerShell 应该使用 `cd apache-dolphinscheduler-2.0.6-src\docker\docker-swarm`
...
请下载二进制包 apache-dolphinscheduler-2.0.6-bin.tar.gz，下载地址: [下载](/zh-cn/download/download.html). 然后将 apache-dolphinscheduler-2.0.6-bin.tar.gz 放到 `apache-dolphinscheduler-2.0.6-src/docker/build` 目录里，在 Terminal 或 PowerShell 中执行:
...
> PowerShell 应该使用 `cd apache-dolphinscheduler-2.0.6-src/docker/build`
