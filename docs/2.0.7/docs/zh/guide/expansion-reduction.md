...
cd /opt
# 解压缩
tar -zxvf apache-dolphinscheduler-2.0.7-bin.tar.gz -C /opt 
cd /opt
mv apache-dolphinscheduler-2.0.7-bin  dolphinscheduler
```
...
```
- 如果扩容的是worker节点,需要设置worker分组.请参安全中心[创建worker分组](./security.md)
