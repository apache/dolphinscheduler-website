...
> PowerShell 应该使用 `cd apache-dolphinscheduler-2.0.3-src\docker\docker-swarm`
...
请参考用户手册章节的[快速上手](/zh-cn/docs/2.0.3/user_doc/guide/quick-start.html)查看如何使用DolphinScheduler
...
-e DATABASE_HOST="192.168.x.x" -e DATABASE_PORT="5432" -e DATABASE_DATABASE="dolphinscheduler" \
-e DATABASE_USERNAME="test" -e DATABASE_PASSWORD="test" \
apache/dolphinscheduler:2.0.3 python-gateway-server
```
...
请下载二进制包 apache-dolphinscheduler-2.0.3-bin.tar.gz，下载地址: [下载](/zh-cn/download/download.html). 然后将 apache-dolphinscheduler-2.0.3-bin.tar.gz 放到 `apache-dolphinscheduler-2.0.3-src/docker/build` 目录里，在 Terminal 或 PowerShell 中执行:
...
> PowerShell 应该使用 `cd apache-dolphinscheduler-2.0.3-src/docker/build`
