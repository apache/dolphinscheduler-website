...
请下载源码包 apache-dolphinscheduler-dev-src.tar.gz，下载地址: [下载](/zh-cn/download/download.html)
...
> PowerShell 应该使用 `cd apache-dolphinscheduler-dev-src\docker\docker-swarm`
...
-p 12345:12345 \
apache/dolphinscheduler:dev all
```
...
-e DATABASE_USERNAME="test" -e DATABASE_PASSWORD="test" \
-e ZOOKEEPER_QUORUM="192.168.x.x:2181" \
apache/dolphinscheduler:dev master-server
```
...
-e DATABASE_USERNAME="test" -e DATABASE_PASSWORD="test" \
-e ZOOKEEPER_QUORUM="192.168.x.x:2181" \
apache/dolphinscheduler:dev worker-server
```
...
-e ZOOKEEPER_QUORUM="192.168.x.x:2181" \
-p 12345:12345 \
apache/dolphinscheduler:dev api-server
```
...
-e DATABASE_HOST="192.168.x.x" -e DATABASE_PORT="5432" -e DATABASE_DATABASE="dolphinscheduler" \
-e DATABASE_USERNAME="test" -e DATABASE_PASSWORD="test" \
apache/dolphinscheduler:dev alert-server
```
...
请下载二进制包 apache-dolphinscheduler-dev-bin.tar.gz，下载地址: [下载](/zh-cn/download/download.html). 然后将 apache-dolphinscheduler-dev-bin.tar.gz 放到 `apache-dolphinscheduler-dev-src/docker/build` 目录里，在 Terminal 或 PowerShell 中执行:

```
$ cd apache-dolphinscheduler-dev-src/docker/build
$ docker build --build-arg VERSION=dev -t apache/dolphinscheduler:dev .
```

> PowerShell 应该使用 `cd apache-dolphinscheduler-dev-src/docker/build`
